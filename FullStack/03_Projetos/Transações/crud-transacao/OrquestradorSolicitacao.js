class OrquestradorSolicitacao {
 ...
 const itensSolicitacaoService = new
ItensSolicitacaoService(conexao);
 const idSolicitacao = await
solicitarCompraService.inserirSolicitacaoCompra(idUsuario);
 await itensSolicitacaoService.inserirItensSolicitacao(idSolicitacao,
itens);
 ...
 static async atualizarSolicitacao(idSolicitacao, idUsuario) {
 const transacao = new TransacaoBanco();
 try {
 await transacao.iniciarTransacao();
 const conexao = transacao.connection;
 const solicitarCompraService = new SolicitarCompraService(conexao);
 await
solicitarCompraService.atualizarSolicitacaoCompra(idSolicitacao, idUsuario);
 await transacao.commit();
 ...
 const solicitarCompraService = new SolicitarCompraService(conexao);
 await
solicitarCompraService.excluirSolicitacaoCompra(idSolicitacao);
 await transacao.commit();
